...
ibdp <- subset(ibdp, idents = c("A3","B3","C3"))

ibdp <- seurat_qc1(ibdp)
ibdp <- seurat_qc2(ibdp)

Idents(ibdp) <- "orig.ident"
ibdp1 <- subset(ibdp, idents = "A3")
ibdp2 <- subset(ibdp, idents = "B3")
ibdp3 <- subset(ibdp, idents = "C3")
...
nExp_poi <- round(0.007*length(ibdp2$orig.ident))
...
ibdp2[["doublet"]]<-ibdp2$DF.classifications_0.25_0.06_7
ibdp3[["doublet"]]<-ibdp3$DF.classifications_0.25_0.06_9


ibdp <- merge(ibdp1, y = c(ibdp2,ibdp3), project = "ibdp_Colon_inflammed")
...
ibdp$pANN_0.25_0.06_7 <- NULL
ibdp$pANN_0.25_0.06_9<- NULL

ibdp$DF.classifications_0.25_0.06_1<- NULL
ibdp$DF.classifications_0.25_0.06_7<- NULL
ibdp$DF.classifications_0.25_0.06_9<- NULL
...
saveRDS(ibdp,paste0("~/ds_group/Charlie Msc project data/IBDP/ibdp_infla_pre_integration.rds") )
